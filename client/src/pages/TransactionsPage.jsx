...
export default function TransactionsPage() {
    const [transactions, setTransactions] = useState([
        { id: 1, date: '2025-08-20', description: 'Grocery Shopping', category: 'Food & Dining', amount: 1250 },
        { id: 2, date: '2025-08-19', description: 'Uber Ride', category: 'Transportation', amount: 500 },
        { id: 3, date: '2025-08-18', description: 'Movie Tickets', category: 'Entertainment', amount: 750 },
        { id: 4, date: '2025-08-17', description: 'Electricity Bill', category: 'Utilities', amount: 2000 },
    ]);
    ...
    const handleAddOrUpdate = () => {
        if (!newTransaction.date || !newTransaction.description || !newTransaction.category || !newTransaction.amount) return;

        if (editTransactionId) {
            setTransactions(prev =>
                prev.map(t =>
                    t.id === editTransactionId ? { ...t, ...newTransaction, amount: Number(newTransaction.amount) } : t
                )
            );
    ...
    return (
        <div className="p-4 sm:p-6 md:p-8 max-w-6xl mx-auto space-y-6">
            ...
            <p className="text-gray-600 dark:text-gray-400 text-sm md:text-base">Track your recent transactions and manage them efficiently.</p>
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                        {transactions.map((tx) => (
                            <tr key={tx.id} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                <td className="px-4 py-2 text-sm text-gray-900 dark:text-white">{tx.date}</td>
                                ...
                                <td className="px-4 py-2 text-sm text-right text-gray-900 dark:text-white">₹{tx.amount.toLocaleString('en-IN')}</td>
                                <td className="px-4 py-2 text-center flex justify-center space-x-2">
                                    <button onClick={() => handleEdit(tx)} className="p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-600">
                                        <Edit className="h-4 w-4 text-indigo-600 dark:text-indigo-400" />
                                    </button>
                                    <button onClick={() => handleDelete(tx.id)} className="p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-600">
